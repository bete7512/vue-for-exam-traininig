  ...
  <v-container>
    <v-card class="mx-auto my-16" max-width="574">
      ...
      <v-form v-model="valid" @submit.prevent="validate">
        <v-text-field justify-center v-model="username" :rules="usernameRules" :counter="10" label="name" required>
        </v-text-field>
        <v-text-field v-model="password" :rules="passwordRules" label="email"  required>
        </v-text-field>
      ...
      <v-row>
        <v-col md6>
          <v-btn class="mr-4 " type="submit" color="red" @click="validate">submit</v-btn>
          </v-col>
          <v-col>
            <v-btn class="mr-4" type="submit" color="red" @click="validate">clear</v-btn>
          </v-col>
      </v-row>
    </v-card>
  </v-container>
